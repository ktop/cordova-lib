...
ConfigFile.prototype.graft_child = function ConfigFile_graft_child(selector, xml_child) {
    ...
    if (self.type === 'xml') {
        ...
        switch (xml_child.mode) {
            case 'merge':
                result = modules.xml_helpers.graftXMLMerge(self.data, xml_to_graft, selector, xml_child.after);
                break;
            case 'replace':
                result = modules.xml_helpers.graftXMLReplace(self.data, xml_to_graft, selector, xml_child.after);
                break;
            case 'delete':
                result = modules.xml_helpers.graftXMLDelete(self.data, xml_to_graft, selector, xml_child.after);
                break;
            default:
...
ConfigFile.prototype.prune_child = function ConfigFile_prune_child(selector, xml_child) {
    ...
    if (self.type === 'xml') {
        ...
        switch (xml_child.mode) {
            case 'merge':
                result = modules.xml_helpers.pruneXMLMerge(self.data, xml_to_graft, selector);
                break;
            case 'replace':
                // TODO: something?
                result = true;
